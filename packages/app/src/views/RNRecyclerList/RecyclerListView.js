...
const NativeRecyclerViewItem = requireNativeComponent('RecyclerListItemView');
...
class RecyclerView extends React.PureComponent {
  ...
  render() {
    ...
    const body = [];
    const itemRangeToRender = this._calcItemRangeToRender(
    ...
    if (itemCount > 0) {
      for (let i = itemRangeToRender[0]; i < itemRangeToRender[1]; i++) {
        const item = dataSource.get(i);
        const itemKey = dataSource.getKey(item, i);
        const shouldUpdate = this._needsItemUpdate(itemKey);
        const isFirst = i == 0;
        const isLast = i == end;
  ...
  _handleVisibleItemsChange = ({ nativeEvent }) => {
    const { firstIndex } = nativeEvent;
    const { lastIndex } = nativeEvent;
  ...
  _calcItemRangeToRender(firstVisibleIndex, lastVisibleIndex) {
    ...
    const count = dataSource.size();
    const from = Math.min(count, Math.max(0, firstVisibleIndex - windowSize));
    const to = Math.min(count, lastVisibleIndex + windowSize);
    return [from, to];
  }
  ...
  _notifyItemRangeInserted(position, count) {
    ...
    } else if (position <= firstVisibleIndex) {
      this.setState({
