...
import type { WebTab } from './TabBarDesktop';
...
function ControllerBarDesktop() {
  ...
  return (
    <XStack bg="$bg" w="100%" h="$12" px="$8" space="$3" alignItems="center">
      <IconButton onPress={() => console.log('goBack')}>
        <IconButton.Icon name="ArrowLeftOutline" />
      </IconButton>
      <IconButton onPress={() => console.log('Go Forward')}>
        <IconButton.Icon name="ArrowRightCircleOutline" />
      </IconButton>
      <IconButton onPress={() => console.log('Reload')}>
        <IconButton.Icon name="ArrowPathUpOutline" />
      </IconButton>
      <XStack>
        <BrowserURLInput
          ref={searchBar}
