...
function ScreenSendEditFee({ ...rest }) {
  ...
  const setFormValuesFromFeeInfo = useCallback(
    ...
    [isEIP1559Fee, isBtcForkChain, setValue, feeInfoPayload?.info.feeDecimals],
  );
  ...
  const btcButtonDisabled = useMemo(() => {
    if (isBtcForkChain) {
      return !formState.isValid;
    }
    return false;
  }, [isBtcForkChain, formState.isValid]);
  ...
  return (
    <BaseSendModal
      ...
      primaryActionProps={{
        isDisabled: feeInfoLoading || btcButtonDisabled,
      }}
